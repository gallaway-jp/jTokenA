...
#define WHAT_ERROR(msg) do {                    \
  ...
  while (0);
    ...
    scoped_ptr<ModelImpl> model(new ModelImpl);
    if (!model->open(param)) {
    ...
    ostream_wrapper ofs(ofilename.c_str());
    if (!*ofs) {
    ...
    if (param.get<bool>("dictionary-info")) {
        for (const DictionaryInfo *d = model->dictionary_info();
             d; d = d->next) {
    ...
    scoped_array<char> ibuf_data(new char[ibufsize]);
    ...
    for (size_t i = 0; i < rest.size(); ++i) {
        istream_wrapper ifs(rest[i].c_str());
        if (!*ifs) {
